...
`tools/list` at `https://gateway.pipeworx.io/radio/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/radio_search_stations \
  -H 'Content-Type: application/json' \
  -d '{"query":"BBC Radio 1"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/radio_search_stations`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
