...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Radio');
}
...
const tools: McpToolExport['tools'] = [
  {
    ...
    description: 'Search radio stations by name. Returns station name, URL, country, genres, and popularity vote count. Use when looking for a specific station or browsing by keyword.',
    summary: 'Internet radio stations matching a name or tag search, from the Radio Browser directory.',
    inputSchema: {
      type: 'object',
      properties: {
        query: {
  ...
  },
  {
    ...
    summary: 'The most-listened internet radio stations right now, from the Radio Browser directory.',
    inputSchema: {
      type: 'object',
      properties: {
        count: {
  ...
  {
    name: 'list_countries',
    description: 'Browse available countries with radio stations. Returns country names and station counts to help target your search geographically.',
    summary: 'The countries with internet radio stations listed, from the Radio Browser directory.',
    inputSchema: {
      type: 'object',
      properties: {},
    },
  ...
  {
    name: 'list_tags',
    description: 'Discover radio genres and tags ranked by station count. Use to explore what categories are available before searching.',
    summary: 'The genre/format tags usable to filter internet radio stations, from the Radio Browser directory.',
    inputSchema: {
      type: 'object',
      properties: {
        limit: {
...
async function searchStations(query: string, limit: number) {
  ...
  if (!res.ok) throw await httpError(res, 'Radio Browser error');

  const data = (await parseJson<unknown>(res, 'Radio Browser')) as RawStation[];
...
async function listCountries() {
  const res = await pwFetch(`${BASE_URL}/countries`);
  if (!res.ok) throw await httpError(res, 'Radio Browser error');

  const data = (await parseJson<unknown>(res, 'Radio Browser')) as RawCountry[];

  // Sort by station count descending and return all entries
...
async function listTags(limit: number) {
  ...
  const res = await pwFetch(`${BASE_URL}/tags?${params.toString()}`);
  if (!res.ok) throw await httpError(res, 'Radio Browser error');

  const data = (await parseJson<unknown>(res, 'Radio Browser')) as RawTag[];
